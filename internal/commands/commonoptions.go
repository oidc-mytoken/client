...
func (opts *commonMTOptsWrap) Common(store bool) commonMTOpts {
	...
	sOpts.SubtokenCapabilities = getCorrectOptsStruct(
		len(sOpts.SubtokenCapabilities) > 0, sOpts, oOpts,
	...
	sOpts.RestrictAudiences = getCorrectOptsStruct(
		len(sOpts.RestrictAudiences.Value()) > 0, sOpts, oOpts,
	).RestrictAudiences
	...
	sOpts.RestrictGeoIPAllow = getCorrectOptsStruct(
		len(sOpts.RestrictGeoIPAllow.Value()) > 0, sOpts, oOpts,
	).RestrictGeoIPAllow
	sOpts.RestrictGeoIPDisallow = getCorrectOptsStruct(
		len(sOpts.RestrictGeoIPDisallow.Value()) > 0, sOpts, oOpts,
	).RestrictGeoIPDisallow
	...
	sOpts.RotationObj.AutoRevoke = getCorrectOptsStruct(
		sOpts.RotationObj.AutoRevoke, sOpts, oOpts,
	).RotationObj.AutoRevoke
